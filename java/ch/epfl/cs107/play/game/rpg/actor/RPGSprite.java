...
public class RPGSprite extends Sprite {
	...
	public RPGSprite(String name, float width, float height, Positionable parent, RegionOfInterest roi, float depth) {
		super(name, width, height, parent, roi, depth);
		this.depthCorrection = 0;
	}
	...
	//Overload for sprite in a 1D tab

	public static  Sprite[] extractSprite(String name, int nbFrames, float width, float height, Positionable parent, int regionWidth, int regionHeight){

		Sprite[] sprites = new Sprite[nbFrames];

		for(int i = 0; i < nbFrames; i++){

			sprites[i]  = new RPGSprite(name, width, height, parent, new RegionOfInterest(i*regionWidth, regionHeight, regionWidth, regionHeight));
